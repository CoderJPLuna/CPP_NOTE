...
- [继承](#继承)
  - [继承的概念及定义](#继承的概念及定义)
    ...
    - [继承的定义](#继承的定义)
      - [定义格式](#定义格式)
      - [继承关系和访问限定符](#继承关系和访问限定符)
      - [继承基类成员访问方式的变化](#继承基类成员访问方式的变化)
  - [基类和派生类对象赋值转换](#基类和派生类对象赋值转换)
  - [继承中的作用域](#继承中的作用域)
  - [派生类的默认成员函数](#派生类的默认成员函数)
  - [继承与友元](#继承与友元)
  - [继承与静态成员](#继承与静态成员)
  - [菱形继承及菱形虚拟继承](#菱形继承及菱形虚拟继承)
  - [继承和组合](#继承和组合)
...
虚拟继承可以解决菱形继承的二义性和数据冗余的问题。如上面的继承关系，在Student和Teacher继承Person时使用虚拟继承，即可解决问题。需要注意的是，虚拟继承不要在其它地方使用。
...
虚拟继承虽然解决了菱形继承产生的二义性和数据冗余问题，但是为了解决一个int数据的冗余却开辟了两个存放虚基表的空间，而且访问虚基类数据时，要通过虚基表进行间接访问，效率会比较低，带来性能上的损耗，所以非必要时尽量避免菱形继承。
